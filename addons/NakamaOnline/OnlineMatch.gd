...
# WebRTC variables:
var _webrtc_peer := WebRTCMultiplayerPeer.new()
...
func _create_webrtc_multiplayer() -> void:
	...
	_webrtc_peer.connect("peer_connected", _on_webrtc_peer_connected)
	_webrtc_peer.connect("peer_disconnected", _on_webrtc_peer_disconnected)
...
func _on_nakama_matchmaker_matched(data: NakamaRTAPI.MatchmakerMatched) -> void:
	...
	_webrtc_peer.create_mesh(players[my_session_id].peer_id)
	multiplayer.multiplayer_peer = _webrtc_peer
...
func _on_nakama_match_state(data: NakamaRTAPI.MatchData) -> void:
	...
	if data.op_code == MatchOpCode.WEBRTC_PEER_METHOD:
		if data.presence.session_id != my_session_id:
			...
			match content['method']:
				...
				'reconnect':
					_webrtc_peer.remove_peer(players[session_id]['peer_id'])
		...
		for session_id in content_players:
			if not players.has(session_id):
				...
				if session_id == my_session_id:
					_webrtc_peer.create_client(players[session_id].peer_id)
					multiplayer.multiplayer_peer = _webrtc_peer
...
func _webrtc_connect_peer(player: Player) -> void:
	...
	webrtc_peer.connect("session_description_created", _on_webrtc_peer_session_description_created)
	webrtc_peer.connect("ice_candidate_created", _on_webrtc_peer_ice_candidate_created)
	
	_webrtc_peers[player.session_id] = webrtc_peer
	
	#get_tree().multiplayer._del_peer(u['peer_id'])
	_webrtc_peer.add_peer(webrtc_peer, player.peer_id)
...
func _on_webrtc_peer_session_description_created(type: String, sdp: String) -> void:
	#var webrtc_peer = _webrtc_peers[session_id]
...
func _on_webrtc_peer_ice_candidate_created(media: String, index: int, name: String) -> void:
